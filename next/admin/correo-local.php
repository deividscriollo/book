...
function activacion_cuenta($correo,$empresa,$ruc,$id){
				   					...
				   					<tbody>
				   						<tr>
											...
											<div style="font-family: Arial, sans-serif; line-height: 36px; color: #444444; font-size: 13px;">
												<a href="http://www.empresa.nextbook.ec/book/processcount.php?activ_reg_count=DKsf984wDMd&id='.$id.'" style="color: #b7837a; text-decoration: none; margin: 0px; text-align: center; vertical-align: baseline; border-width: 1px 1px 2px; border-style: solid; border-color: #d7a59d; padding: 6px 12px; font-size: 14px; line-height: 20px; background-color: #ffffff;">Clic aquí para activar la cuenta</a>
											</div>
				   							</td>
				   						</tr>
				   					</tbody>
	...
	';
	// Contenido
	$titulo = utf8_decode('Activación cuenta para uso de la aplicación');
	$titulo2 = utf8_decode('Activación cuenta nextbook');
	$contenido_html=utf8_decode($contenido_html);
	$email = new email();
	// Mail it
	$acu=0;
	if ( $email->enviar( $correo , $titulo , $titulo2 ,  $contenido_html ) ){
		$acu=1;
	}
	return $acu;
}
function activacion_correo($correo,$empresa,$ruc,$id, $user, $passwor){
	...
	$contenido_html='
		...
		';
	$titulo = utf8_decode('Activación cuenta para uso de la aplicación');
	$titulo2 = utf8_decode('Activación correo nextbook');
	$contenido_html=utf8_decode($contenido_html);
	$email = new email();
	$acu=0;
	if ( $email->enviar( $correo , $titulo , $titulo2 ,  $contenido_html ) ){
