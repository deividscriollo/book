<?php
    ...
    $count = 0;
    $resultado = $class->consulta("select  COUNT(*) AS count from facturanext.correo as FC where FC.id_usuario = '".$_SESSION['id_miempresa']."' and FC.stado = '1' UNION ALL select  COUNT(*) AS count from facturanext.correo as FC where FC.id_usuario = '".$_SESSION['id_miempresa']."' and FC.stado = '5'");        
    ...
    if ($search == 'false') {
        $SQL = "SELECT FC.id,FC.tipo_consumo,FC.razon_social,FC.tipo,FC.fecha_correo,FC.remitente,FC.id_usuario,FF.fecha_emision  from facturanext.correo as FC, facturanext.facturas as FF where FC.id = FF.id_correo and FC.id_usuario = '".$_SESSION['id_miempresa']."'  and FC.stado = '1' UNION ALL SELECT  FC.id,FC.tipo_consumo,FC.razon_social,FC.tipo,FC.fecha_correo,FC.remitente,FC.id_usuario,FF.fecha_emision  from facturanext.correo as FC, facturanext.facturas_fisica as FF where FC.id = FF.id_correo and FC.id_usuario = '".$_SESSION['id_miempresa']."' and FC.stado = '5' ORDER BY  $sidx $sord offset $start limit $limit";
    } else {
