<?php
	...
	// ------------------------------------------------ Lib requiered ------------------------------------------------------ //
	include_once('../../admin/fpdf/rotation.php');
	include_once('../../admin/fpdf/barcode.inc.php');
    ...
    $id = $_GET['id'];
    $ext = $_GET['ext'];
    $id_miempresa = $_GET['id_miempresa'];

	// ----------------------------------------- Herencia contstructor conexion db ----------------------------------------- //
	$class = new constante();
	$pFile = "../archivos/".$id_miempresa."/".$id.".".$ext;
	...
	class PDF extends PDF_Rotate{   
        ...
        function Header(){                         
            ...
            //$this->SetX(1);
            $this->SetY(1);
            $this->Cell(30, 5, 'Generado en: '.$fecha, 0,0, 'C', 0);                                                             
            $this->Cell(168, 5, 'www.facturanext.com', 0,10, 'R', 0);                                                             
            $this->Ln(7);
            $this->SetX(13);
            $this->RotatedImage('../../admin/fpdf/logo.fw.png', 30, 232, 200, 30, 45);                    
            $this->SetX(0);            
		}
        function Footer(){            
            $this->SetY(-10);            
	...
	if($xmlAut->infoTributaria->codDoc == '01'){
	    ...
	    $pdf->Text(108, 64, utf8_decode('AMBIENTE: '. utf8_encode($ambi)));//ambiente
