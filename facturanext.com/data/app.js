...
jQuery(function($) {	
    ...
    jQuery(grid_selector_3).jqGrid({	        
        datatype: "xml",
        url: 'data/xml_facturas_fisicas.php',        
	...
	$('#form_proceso').validate({
		...
		submitHandler: function (form) {
			...
			$.ajax({       
		    	...
		    	type: "POST",
		    	dataType: 'json',        
		    	url: "data/mod_cell.php?fn=3&id="+id+"&acceso="+$("#txt_clave").val()+"&consumo="+$("#slt_consumo").val(),        
		    	success: function(data, status, jqXHR) {      		
		    		if(data == 1) {
		    			$.gritter.add({
...
function nuevos_mensajes() {	
	jQuery.ajax({
		type: 'POST',
		url: 'data/mod_cell.php?fn=4',		
		dataType: 'json',
...
// 	jQuery.ajax({
// 		type: 'POST',
// 		url: 'data/mod_cell.php?fn=5&session='+session,		
// 		dataType: 'json',
// 		success: function(retorno) {
// 			// if(retorno == 1){
			...
			if($('#txt_m_2').val() == '') {
				...
				if($('#txt_m_3').val() == '') {
					...
					$.ajax({       
				    	...
				    	type: "POST",
				    	dataType: 'json',        
				    	url: "data/mod_cell.php?fn=7&ruc="+$('#txt_m_1').val()+"&nombre="+$("#txt_m_2").val()+"&dir="+$("#txt_m_3").val()+"&com="+$("#txt_m_4").val(),        
				    	success: function(data, status) {      		
				    		if(data == 1) {		
				    			$.gritter.add({
				...
				if($("#txt_4").val() == '') {
					...
					if(tam.length == 0) {
						...
						$.ajax({       
					    	...
					    	url: "data/mod_cell.php?fn=8&id="+id,
					    	success: function(data, status) {      		
					    		if(data == 1) {
					    			$.gritter.add({
