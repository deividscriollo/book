<?php
    ...
    $count = 0;
    $resultado = $class->consulta("select  COUNT(*) AS count from facturanext.facturas_fisica F where F.id_usuario = '".$_SESSION['modelo']['empresa_id']."'");        
...
while ($row = pg_fetch_row($resulto)) {
    $s .= "<row id='" . $row[0] . "'>";
    $s .= "<cell>" . $row[0] . "</cell>";
    $s .= "<cell>" . $row[1] . "</cell>";
    $s .= "<cell>" . $row[2] . "</cell>";
