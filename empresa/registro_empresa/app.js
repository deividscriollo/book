jQuery(function($) {
	...
	$('#txt_user_dc').inputmask({mask: "9999999999001@FACTURANEXT.com"});
	$('#txt_movil_usuario').inputmask({mask: "9999999999001@FACTURANEXT.com"});
	...
	$('#myform').validate({
        ...
        submitHandler: function (form) { // for demo
			...
			$.ajax({
				...
				success: function (data) {
					if (data[0]==0) {
						bootbox.alert("Usuario o contraseña no validos");
					};
					if (data[0]==1) {
						location.href="../";
					}
	...
	$('#form_movil').validate({
        ...
        submitHandler: function (form) { // for demo
        	$('.pass_olvidado').removeClass('hide');
        	var usuario=$('#txt_movil_usuario').val();
			var password=$('#txt_movil_pass').val();
			$.ajax({
				url: 'app.php',
				type: 'post',
				dataType:'json',
